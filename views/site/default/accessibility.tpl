<link rel="stylesheet" href="/static/3rd_party/others/css/keys.css" property='stylesheet'>
<div class="page-header">
  <div class="row" >
    <div class="col-lg-12 col-md-12 col-sm-12" >
      <div class="panel panel-default">
        <div class="panel-heading" style="padding-bottom:0px">
          <ul class="breadcrumb">
            <li><a href="/">Home</a></li>
            <li class="active">Accessibility</li>
          </ul></div>
          <div class="panel-body">
            <h1>Accessibility</h1>
            <blockquote>
              <p>The power of the Web is in its universality. Access by everyone regardless of disability is an essential aspect.</p>
              <small>Tim Berners-Lee, W3C Director and inventor of the <cite title="Source Title">World Wide Web</cite></small>
            </blockquote>
            <div>
              <div>
                <h3>FAQ</h3>
                <div>
                  <ol>
                    <li><a href="#accessibility-answers-1">Can I navigate on the website, using just the keyboard?</a></li>
                    <li><a href="#accessibility-answers-2">Do you provide shortcuts?</a></li>
                  </ol>
                </div>
              </div>
              <div>
                <h4>Answers</h4>
                <div id="accessibility-answers">
                  <div id="accessibility-answers-1" class="well">
                    <h5>Can I navigate on the website, using just the keyboard?</h5>
                    <span>Yes, you can. In order to navigate using the keyboard, you can use these combinations of keys:</span>
                    <ul class="light-keys">
                      <li>
                        <kbd>SPACE</kbd> to scroll down
                      </li>
                      <li>
                        <kbd>Up</kbd> <kbd>Down</kbd> <kbd>Left</kbd> <kbd>Right</kbd> arrows to scroll in that direction
                      </li>
                    </ul>
                    <ul class="light-keys">
                      <li >
                        <kbd>TAB</kbd> to start focusing an element and to focus the next element
                      </li>
                      <li>
                        <kbd>Shift</kbd> + <kbd>TAB</kbd> to focus the previous element
                      </li>
                      <li>
                        An element which is focused has a blue border around it.
                        <ul class="light-keys">
                          <li>
                            <kbd>Space</kbd> to choose a focused element
                          </li>
                          <li>
                            <kbd>Enter</kbd> to access the focused element
                          </li>
                        </ul>
                        <br />
                        <figure>
                          <figcaption>Example of an element focused - <i>Modify</i> is focused</figcaption>
                        <img class="img-responsive img-thumbnail"  src="/static/img/default/accessibility/example-focused-element.jpg" alt="Example of an element focused" />
                      </figure>
                      </li>
                    </ul>
                  </div>
                  <div id="accessibility-answers-2" class="well">
                    <h5>Do you provide shortcuts?</h5>
                    <span>Yes, you can use shortcuts. This is a list of shortcuts:</span>
                    <ul>
                      <li> All pages
                        <ul class="light-keys">
                          <li>
                            <kbd>ALT</kbd> + <kbd>A</kbd> It access the accessibility page
                          </li>
                          <li>
                            <kbd>ALT</kbd> + <kbd>H</kbd> It access the home page
                          </li>
                          <li>
                            <kbd>ALT</kbd> + <kbd>C</kbd> It accesss the contact page
                          </li>
                        </ul>
                      </li>
                      <li> When you are connected
                        <ul class="light-keys">
                          <li>
                            <kbd>ALT</kbd> + <kbd>L</kbd> It logs you out
                          </li>
                        </ul>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
