<link rel="stylesheet" href="/static/3rd_party/others/css/keys.css" property='stylesheet'>
<div class="page-header">
  <div class="row">
    <div class="col-lg-4 col-md-5 col-sm-5">
      <div class="panel panel-default" >
        <div class="panel-heading">
          <h3 class="panel-title">How can we help you?</h3>
        </div>
        <div class="panel-body">
          <ul>
            <li><a class="scroll" href="#FAQ">FAQ</a></li>
            <li><a class="scroll" href="#legal-aspects">Legal aspects</a></li>
          </ul>
        </div>
      </div>
    </div>
    <div class="col-lg-8 col-md-7 col-sm-7">
      <div class="panel panel-default">
        <div class="panel-heading" style="padding-bottom:0px">
          <ul class="breadcrumb">
            <li><a href="/">Home</a>
            </li>
            <li class="active">Help center</li>
          </ul>
        </div>
        <section class="panel-body">
          <h1>Help</h1>
          This page contain information which may help you.
          <section>
            <h2 id="FAQ">FAQ</h2>
            <ol>
              <li>
                <a class="scroll" href="#help-answers-1">I need an email to log in. Why don't you use an username?
                </a>
              </li>
              <li>
                <a class="scroll" href="#help-answers-2">Can I obtain the data that Wisply is using?
                </a>
              </li>
              <li>
                <a class="scroll" href="#help-answers-3">How does Wisply work?
                </a>
              </li>
              <li>
                <a class="scroll" href="#help-answers-4">How do I integrate my repository with Wisply?
                </a>
              </li>
              <li>
                <a class="scroll" href="#help-answers-5">How can I remove a resource from Wisply?
                </a>
              </li>
              <li>
                <a class="scroll" href="#help-answers-6">The content for some resources is not visible. Why?
                </a>
              </li>
            </ol>
            <section id="help-answers">
              <h2>Answers</h2>
              <article id="help-answers-1" class="well">
                <h3>I need an email to log in. Why don't you use an username?</h3>
                We had to decide which method is better. We have chosen the combination of a real name and an email for various reasons. Firstly, the real name idenfity a person in the virtual space better than a combination of letters and digits. A username can be forgetten while an email is frequently used, so it may be more covinient for you to use it.
              </article>
              <article id="help-answers-2" class="well">
                <h3>Can I obtain the data that Wisply is using?</h3>
                Of course! <br />
                There are two main ways to get Wisply data <br />
                <ol>
                  <li>HTTP requests using our Application Programming Interface (API). The list of requests is on <a href="developer#http">developer page</a>.</li>
                  <li>You can download the entire tables which Wisply is using. <a class="btn btn-xs btn-primary" href="developer/table/list">See list of tables</a></li>
                </ol>
              </article>
              <article id="help-answers-3" class="well">
                <h3>How does Wisply work?</h3>
                Great question! <br />
                Wisply has tried to automate the process of collecting and analysing data from different institutions.<br />
                <br />
                <br />
                <figure>
                <img src="/static/img/public/about/about.png" class="img-responsive" alt="Diagram which explains how Wisply works" />
                <figcaption>
                  How is Wisply working?
                </figcaption>
                </figure>
                <br />
                <br />
                For more information, please see <a href="about">about page</a>.
              </article>
              <article id="help-answers-4" class="well">
                <h3>How do I integrate my repository with Wisply?</h3>
                <br />
                In case you want to integrate your repository with Wisply, you can <a href="/contact">contact us</a>. We will make sure that Wisply will gather metadata data from it.
              </article>
              <article id="help-answers-5" class="well">
                <h3>How can I remove a resource from Wisply?</h3>
                <br />
                We apologize for this inconvenience. Wispy is collecting in an automate way data from open repositories.
                <br />
                We have a take down policy regarding the data, which you can read it <a href="/take-down-policy">here</a>.
                <br />
                In case you consider that a resource should not appear on Wisply, please <a href="/contact">contact us</a> and we will remove it.
              </article>
              <article id="help-answers-6" class="well">
                <h3>The content for some resources is not visible. Why?</h3>
                <br />
                Wisply is collecting metadata about all the resources from a repository. The metadata may include the date, the creators, the formats and many other details. As you see, Wisply does not get the actual files.
                <br /><br />
                For some resources, Wisply has a mechanism which collects the previous images. This mechanism helps the users to get an idea about the resource.
                <br /><br />
                <b>Why not for all?</b>
                <br />
                There are two reasons:
                <br />
                <ul>
                  <li> The content can't be displayed in the browser</li>
                  <li> The resource is not public, thus is not visible to Wisply</li>
                </ul>
              </article>
            </section>
          </section>
          <section>
            <h2 id="legal-aspects">Legal aspects</h2>
            <ol>
              <li><h3 class="h6"><a href="/privacy">Privacy Policy</a></h3></li>
              <li><h3 class="h6"><a href="/cookies">Cookies Policy</a></h3></li>
              <li><h3 class="h6"><a href="/terms-and-conditions">Terms and conditions</a></h3></li>
              <li><h3 class="h6"><a href="/take-down-policy">Take down policy</a></h3></li>
            </ol>
          </section>
        </section>
      </div>
    </div>
  </div>
</div>
